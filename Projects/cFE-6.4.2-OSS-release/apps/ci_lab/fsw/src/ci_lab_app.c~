    ...
    */
    while (CFE_ES_RunLoop(&RunStatus) == TRUE)
    {
        ...
        status = CFE_SB_RcvMsg(&CIMsgPtr, CI_CommandPipe, 500);
    		...
    		CFE_EVS_SendEvent (CI_STARTUP_INF_EID, CFE_EVS_INFORMATION,"Run Loop %d",status);
        
        CFE_ES_PerfLogEntry(CI_MAIN_TASK_PERF_ID);

        if (status == CFE_SUCCESS)
        {
        		CFE_EVS_SendEvent (CI_STARTUP_INF_EID, CFE_EVS_INFORMATION,"Command Received");
...
void CI_ReadUpLink(void)
{
    ...
    CFE_EVS_SendEvent (CI_STARTUP_INF_EID, CFE_EVS_INFORMATION,"ReadUplink");
